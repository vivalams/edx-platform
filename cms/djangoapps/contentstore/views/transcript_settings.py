...
def transcript_add(user, edx_video_id, transcript_file, language_code):
    video = Video.objects.filter(edx_video_id=edx_video_id).first()

    if video:
        try:
            course_id = video.courses.first().course_id
        except AttributeError as e:
            return JsonResponse({"error": e.message}, status=400)

        course = get_course_and_check_access(CourseKey.from_string(course_id), user)
    ...
    return JsonResponse({"error": _("Video does not exist")}, status=400)
