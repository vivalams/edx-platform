...
from .models import RestrictedApplication, ScopedOrganization,ScopedApplication
...
class _DispatchingView(View):
    ...
    def is_application_scopes_restricted(self, request):
        ...
        if ScopedApplication.objects.filter(id=dot_id).exists():
            return True
        else:
            return False
    ...
    def get_associated_orgs(self, request):
        ...
        if ScopedOrganization.objects.filter(application_id=dot_id).exists():
            return ScopedOrganization.objects.get(application_id=dot_id).org_associations
        else:
            return None
