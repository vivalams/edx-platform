...
from .models import RestrictedApplication, RestrictedApplication_33
...
class _DispatchingView(View):
    ...
    def is_application_restricted(self, request):
        """
        Returns the appropriate adapter based on the OAuth client linked to the request.
        """
    ...
    def get_associated_orgs(self, request):
        ...
        """
        dot_id = self._get_application_id(request)
        if RestrictedApplication_33.objects.filter(application_id=dot_id).exists():
            return RestrictedApplication_33.objects.get(application_id=dot_id).org_associations
        else:
            return None
...
class AccessTokenView(RatelimitMixin, _DispatchingView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if response.status_code == 200 and request.POST.get('token_type', '').lower() == 'jwt':
            ...
            is_application_restricted = self.is_application_restricted(request)
            #if is_application_restricted:
            org = self.get_associated_orgs(request)	
            #else:
            #org = None
            application_grant_type = self.get_application_grant_type(request)
            content = {
                'access_token': JwtBuilder(user).build_token(scopes, expires_in, org=org, application_grant_type=application_grant_type,is_application_restricted=is_application_restricted),
