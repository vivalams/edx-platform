...
def _track_event(event_name, bookmark):
    ...
    if settings.FEATURES.get('SQUELCH_PII_IN_LOGS', False):
        context_override['path'] = string.replace(context_override['path'], bookmark.user.username, '')
        bookmark_id = string.replace(bookmark.resource_id, bookmark.user.username, '')
