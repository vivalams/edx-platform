...
@patch.dict(settings.FEATURES, {'ENABLE_BULK_GRADES_API': True})
@ddt.ddt
...
class CourseGradesViewTest(GradeViewTestMixin, APITestCase):
    ...
    def setUp(self):
        super(CourseGradesViewTest, self).setUp()
    ...
    def test_staff_can_get_all_grades(self, staff_user):
        self.client.logout()
        self.client.login(username=getattr(self, staff_user).username, password=self.password)
        resp = self.client.get(self.get_url())
        ...
        expected_data = [
            ...
            {
                'user': self.other_student.username,
                'course_key': str(self.course.id),
                'passed': False,
                'percent': 0.0,
                'letter_grade': None
